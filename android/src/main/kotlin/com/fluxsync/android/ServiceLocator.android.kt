...
object ServiceLocator {
    ...
    fun init(context: Context, scope: CoroutineScope) {
        ...
        AndroidDiscoveryEnvironment.context = appContext
        val settings = SharedPreferencesSettings(appContext.getSharedPreferences("fluxsync", Context.MODE_PRIVATE))
        ...
        val localPeer = Peer("android-local", moniker, false, "0.0.0.0", ProtocolConstants.WS_PORT)
        val registry = PeerRegistry(settings)
        val discovery = PeerDiscovery(scope)
        val trustStore = TrustStore(settings)
        val pairing = PairingSession(scope)
        val ackTracker = AckTracker()
        val dataClient = DataChannelClient()
        ...
        LogManager.init("${appContext.filesDir.absolutePath}/logs", scope)
        ...
        viewModelStore = ViewModelStore(
            ...
            transferViewModel = TransferViewModel(
                ...
                navigator = navigator,
                transfersDirectory = "${appContext.filesDir.absolutePath}/transfers",
